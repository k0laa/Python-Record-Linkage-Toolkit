...
class ConfigReader:
    ...
    def get_results_database_path(self):
        ...
        db_name = output_config.get('results_database_name', None)
        if db_name:
            return db_name
        source_name = self.get_source_database()['path'].split('/')[-1].replace('.db', '')
        target_name = self.get_target_database()['path'].split('/')[-1].replace('.db', '')
        return f"../data/results/{source_name}_{target_name}_results.db"
